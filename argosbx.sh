...
agsbxurl="https://raw.githubusercontent.com/zxkke/argo/refs/heads/main/argosbx.sh"
showmode(){
echo "Argosbx脚本一键SSH命令生器在线网址：https://zxkke.github.io/argo/"
echo "主脚本：bash <(curl -Ls https://raw.githubusercontent.com/zxkke/argo/refs/heads/main/argosbx.sh) 或 bash <(wget -qO- https://raw.githubusercontent.com/zxkke/argo/refs/heads/main/argosbx.sh)"
...
echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
echo "修改版Github项目 ：github.com/zxkke/argo"
echo "原作者Github项目 ：github.com/yonggekkk"
echo "原作者Blogger博客：ygkkk.blogspot.com"
...
echo "正在安装必要依赖: unzip"
apk add --no-cache unzip >/dev/null 2>&1 || echo "警告：unzip 安装失败，可能影响解压"
fi
# 检查 gcompat（glibc 兼容层）
if ! apk info gcompat >/dev/null 2>&1; then
echo "警告：Alpine Linux 使用 musl libc，官方 Xray/Sing-box 可能不兼容"
echo "建议安装 gcompat（glibc 兼容层）: apk add gcompat"
...
echo "从 Xray 官方下载版本：$xray_version"
...
if command -v unzip >/dev/null 2>&1; then
  unzip -q -o "$HOME/agsbx/xray.zip" -d "$HOME/agsbx/" xray 2>/dev/null || echo "解压失败"
  rm -f "$HOME/agsbx/xray.zip"
else
  echo "警告：未安装unzip，尝试使用 tar 解压"
  # 尝试直接解压 zip（某些系统的 tar 支持 zip）
  tar -xf "$HOME/agsbx/xray.zip" -C "$HOME/agsbx/" xray 2>/dev/null && rm -f "$HOME/agsbx/xray.zip" || {
    echo "错误：无法解压 Xray 文件，请安装 unzip: apk add unzip 或 apt install unzip"
    rm -f "$HOME/agsbx/xray.zip"
    exit 1
  }
fi
...
echo "从 Sing-box 官方下载版本：$sb_version"
...
tar -xzf "$HOME/agsbx/sing-box.tar.gz" -C "$HOME/agsbx/" --strip-components=1 sing-box-${sb_version#v}-linux-${sb_arch}/sing-box 2>/dev/null || echo "解压失败"
...
if [ ! -f "$HOME/agsbx/private.key" ] || [ ! -f "$HOME/agsbx/cert.pem" ]; then
  if command -v openssl >/dev/null 2>&1; then
    ...
    echo "已使用 OpenSSL 生成自签名证书"
  else
    echo "错误：未安装 OpenSSL，无法生成证书文件"
    echo "请安装 OpenSSL: apt install openssl 或 yum install openssl"
    exit 1
  fi
fi
if [ -n "$hyp" ]; then
