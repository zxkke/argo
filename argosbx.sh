...
if command -v unzip >/dev/null 2>&1; then
  unzip -q -o "$HOME/agsbx/xray.zip" -d "$HOME/agsbx/" xray
  mv "$HOME/agsbx/xray" "$HOME/agsbx/xray" 2>/dev/null || true
  rm -f "$HOME/agsbx/xray.zip"
else
  echo "警告：未安装unzip，尝试下载预编译单文件版本"
  url="https://github.com/XTLS/Xray-core/releases/download/${xray_version}/xray-linux-${cpu}.tar.gz"
  (command -v curl >/dev/null 2>&1 && curl -Lo "$HOME/agsbx/xray.tar.gz" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 10 wget -O "$HOME/agsbx/xray.tar.gz" --tries=2 "$url")
  tar -xzf "$HOME/agsbx/xray.tar.gz" -C "$HOME/agsbx/" xray 2>/dev/null || echo "解压失败"
  rm -f "$HOME/agsbx/xray.tar.gz"
fi
